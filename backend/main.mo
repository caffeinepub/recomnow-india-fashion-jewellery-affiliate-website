...
actor {
  ...
  // Check if caller is admin via AccessControl (Internet Identity path)
  ...
  // Check if caller is admin via active session (custom username/password path)
  func isSessionAdmin(caller : Principal) : Bool {
    for ((token, session) in sessions.entries()) {
      let now = Time.now();
      if (session.principal == caller and now < session.expiresAt and session.username == "admin") {
        Debug.print("Session admin found for principal: " # caller.toText());
        return true;
      };
    };
    false;
  ...
  public shared ({ caller }) func addProduct(input : ProductInput) : async Nat {
    ...
    // Only admins can add products (admin panel functionality)
    if (not isAdmin(caller)) {
